<template>
  <div class="cardsMain" v-if="Object.keys(currentArticle).length>0" >
        ...
        <div class="imgArticle">
            <video v-if="currentArticle.fileType==='video'" :src="'./api/public/articlefile/'+currentArticle.filePath" controls /> 
            <img v-else :src="'./api/public/articlefile/'+currentArticle.filePath" alt="l'article">
        </div>
        <div class="imgUser">
        <img :src="'./api/public/userprofile/'+currentArticle.profilPhoto" alt="user">
...
export default {
    ...
    methods:{
        ...
        getArticleById : async function (idArticle) {
            const request = await fetch("./api/index.php?controller=ArticlesController&action=getarticlebyid&id="+idArticle)
